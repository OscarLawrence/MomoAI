...
class GraphBackend:
    ...
    async def insert_node(self, node: GraphNode) -> GraphDiff:
    ...
    async def insert_edge(self, edge: GraphEdge) -> GraphDiff:
        """Insert an edge and create diff record."""
        # Add to runtime tier (hottest)
        self._storage.add_edge(edge, GraphStorageTier.RUNTIME)

        # Create diff record
        diff = GraphDiff(operation=GraphDiffType.INSERT_EDGE, edge=edge)
        self._diff_history.append(diff)
    ...
    async def delete_node(self, node_id: str) -> GraphDiff:
        ...
        # Create diff record
        diff = GraphDiff(operation=GraphDiffType.DELETE_NODE, node=node)
        self._diff_history.append(diff)
    ...
    async def delete_edge(self, edge_id: str) -> GraphDiff:
        ...
        # Create diff record
        diff = GraphDiff(operation=GraphDiffType.DELETE_EDGE, edge=edge)
        self._diff_history.append(diff)
    ...
    async def _apply_diff(self, diff: GraphDiff) -> None:
        ...
        if diff.operation == GraphDiffType.INSERT_NODE and diff.node:
            self._storage.add_node(diff.node, GraphStorageTier.RUNTIME)
        elif diff.operation == GraphDiffType.DELETE_NODE and diff.node:
            for tier in [
                GraphStorageTier.RUNTIME,
                GraphStorageTier.STORE,
                GraphStorageTier.COLD,
        ...
        elif diff.operation == GraphDiffType.INSERT_EDGE and diff.edge:
            self._storage.add_edge(diff.edge, GraphStorageTier.RUNTIME)
        elif diff.operation == GraphDiffType.DELETE_EDGE and diff.edge:
            for tier in [
                GraphStorageTier.RUNTIME,
                GraphStorageTier.STORE,
                GraphStorageTier.COLD,
    ...
    async def prune(
        ...
        if runtime_limit is not None:
            pruned = self._storage.prune_by_size_limit(
                GraphStorageTier.RUNTIME, runtime_limit, GraphStorageTier.STORE
            )
            total_pruned += pruned
        ...
        if store_limit is not None:
            pruned = self._storage.prune_by_size_limit(
                GraphStorageTier.STORE, store_limit, GraphStorageTier.COLD
            )
            total_pruned += pruned
