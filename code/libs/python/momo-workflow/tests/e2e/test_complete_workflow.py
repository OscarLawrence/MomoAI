...
class TestCompleteWorkflowScenarios:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            class FileProcessingStep(BaseWorkflowStep):
                def __init__(self, step_id: str, filename: str, content: str):
                    super().__init__(
                        step_id, f"Process file: {filename}", reversible=True
                    )
                ...
                def rollback(
                    self, context: WorkflowContext, result: StepResult
                ) -> None:
            ...
            steps = [
                FileProcessingStep(
                    "process_input", "input.txt", "Line 1\nLine 2\nLine 3"
                ),
                FileProcessingStep(
                    "process_config", "config.txt", "Setting 1\nSetting 2"
                ),
            ]
    ...
    def test_context_data_flow(self):
        ...
        steps = [
            DataFlowStep(
                "init_step", output_var="initial_data", transform_func=lambda x: 10
            ),
            DataFlowStep(
                "multiply_step", "initial_data", "multiplied_data", lambda x: x * 2
            ),
