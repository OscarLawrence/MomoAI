...
class WorkflowBenchmarkSuite:
    ...
    def run_full_benchmark_suite(self) -> Dict[str, Any]:
        ...
        benchmark_results["summary"] = self._generate_summary_statistics(benchmark_results["benchmarks"])
    ...
    def _benchmark_execution_performance(self) -> Dict[str, Any]:
        ...
        if len(step_counts_with_data) > 1:
            ...
            scaling_factor = self._calculate_scaling_factor(step_counts_with_data, mean_durations)
            results["analysis"]["scaling_factor"] = scaling_factor
            results["analysis"]["performance_category"] = self._categorize_performance(scaling_factor)
    ...
    def _benchmark_scalability(self) -> Dict[str, Any]:
        ...
        for step_count in large_step_counts:
            ...
            try:
                ...
                if duration > 30.0:  # 30 second timeout
                    results["analysis"]["early_termination"] = f"Terminated at {step_count} steps due to timeout"
                    break
        ...
        for measurement in results["measurements"].values():
            if "steps_per_second" in measurement and measurement["steps_per_second"] > 0:
                throughputs.append(measurement["steps_per_second"])
        
        if throughputs:
            results["analysis"]["mean_throughput"] = statistics.mean(throughputs)
            results["analysis"]["throughput_stability"] = statistics.stdev(throughputs) / statistics.mean(throughputs)
            ...
            for failure_point in [2, workflow_size // 2, workflow_size - 2]:
                ...
                if rollback_durations:
                    results["measurements"][test_key] = {
                        ...
                        "rollback_overhead": statistics.mean(rollback_durations) / workflow_size,
    ...
    def _benchmark_memory_efficiency(self) -> Dict[str, Any]:
        ...
        for scenario_name, step_count, step_duration in test_scenarios:
            ...
            workflow, context = create_performance_test_scenario(step_count, step_duration)
            ...
            results["measurements"][scenario_name] = {
                ...
                "memory_per_step": (result.overall_metrics.memory_peak_mb or 0) / step_count,
                "success_rate": result.success_rate
            }
        ...
        for command_type in command_types:
            ...
            if durations:
                results["measurements"][command_type] = {
                    "mean_duration": statistics.mean(durations),
                    "overhead_std": statistics.stdev(durations) if len(durations) > 1 else 0,
                    "sample_size": len(durations)
                }
        
        return results
    
    def _calculate_scaling_factor(self, step_counts: List[int], durations: List[float]) -> float:
    ...
    def _generate_summary_statistics(self, benchmarks: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "execution_performance" in benchmarks and "analysis" in benchmarks["execution_performance"]:
            exec_analysis = benchmarks["execution_performance"]["analysis"]
            if "performance_category" in exec_analysis:
                summary["performance_highlights"]["execution_category"] = exec_analysis["performance_category"]
        
        if "scalability_analysis" in benchmarks and "analysis" in benchmarks["scalability_analysis"]:
            scale_analysis = benchmarks["scalability_analysis"]["analysis"]
            if "mean_throughput" in scale_analysis:
                summary["performance_highlights"]["mean_throughput_steps_per_sec"] = scale_analysis["mean_throughput"]
