...
def demonstrate_command_registry():
    ...
    if config_cmd:
        ...
        result = config_cmd.execute(filename="workflow_config.json", config_data=config_data)
...
def create_shell_command_examples():
    ...
    echo_result = echo_command.execute(
        message="Hello from shell command!",
...
def main():
    ...
    context = WorkflowContext(
        working_directory=Path.cwd() / "command_demo_output"
    )
