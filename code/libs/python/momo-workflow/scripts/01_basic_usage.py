...
class HelloWorldStep(BaseWorkflowStep):
    ...
    def __init__(self, name: str = "World"):
        super().__init__(
            step_id="hello_world", description=f"Say hello to {name}", reversible=False
        )
        self.name = name
...
def create_basic_workflow() -> WorkflowDefinition:
    ...
    steps = [
        ...
        FileCreationStep(
            "workflow_output.txt", "This file was created by the workflow!"
        ),
...
def main():
    ...
    for step_result in result.step_results:
        ...
        print(
            f"  {status_emoji} {step_result.step_id}: {step_result.metrics.duration_seconds:.3f}s"
        )
    ...
    if result.artifacts_produced:
        ...
        for artifact in result.artifacts_produced:
            if artifact.exists():
                content_preview = (
                    artifact.read_text()[:50] + "..."
                    if len(artifact.read_text()) > 50
