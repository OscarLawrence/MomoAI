...
def demonstrate_custom_test_scenario():
    ...
    for i, step in enumerate(steps):
        if hasattr(step, 'executed'):
            ...
            rollback_status = "🔄 Rolled back" if getattr(step, 'rolled_back', False) else ""
...
def run_performance_benchmarks():
    ...
    try:
        from momo_workflow.benchmarks.performance_benchmarks import WorkflowBenchmarkSuite
...
def demonstrate_rollback_testing():
    ...
    rollback_steps.append(TestWorkflowStep(
        "failing_step",
        "Step that fails to trigger rollback",
        should_fail=True
    ))
    ...
    for i, step in enumerate(rollback_steps[:-1]):  # Exclude failing step
        rollback_status = "🔄 Rolled back" if getattr(step, 'rolled_back', False) else "⏸️  Not rolled back"
