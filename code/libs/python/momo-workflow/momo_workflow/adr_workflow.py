...
class ADRWorkflowAgent:
    ...
    def create_branch_and_setup(self, short_name: str, title: str) -> str:
        ...
        subprocess.run(
            ["git", "checkout", "-b", branch_name], cwd=self.project_root, check=True
        )
        ...
        self._create_from_template(
            adr_path / "implementation-plan.md",
            "implementation_plan",
            {
                "number": adr_number,
                "title": title,
    ...
    def continue_workflow(self, adr_number: str):
        ...
        adr_folders = [
            d
            for d in self.adr_dir.iterdir()
            if d.is_dir() and d.name.startswith(adr_number)
        ]
    ...
    def finalize_adr(self, adr_number: str):
        ...
        if decision_file.exists():
            ...
            content = content.replace(
                "**Status:** DRAFT", "**Status:** ✅ **ACCEPTED & IMPLEMENTED**"
            )
...
🤖 Generated with [Claude Code](https://claude.ai/code)"""
    ...
    def _implementation_phase(self, adr_path: Path, state: Dict):
        ...
        if not log_file.exists():
            self._create_from_template(
                log_file,
                "implementation_log",
                {"number": state["adr_number"], "title": state["title"]},
            )
    ...
    def _create_from_template(
        self, file_path: Path, template_name: str, variables: Dict
    ):
