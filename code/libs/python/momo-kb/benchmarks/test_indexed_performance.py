...
async def benchmark_indexed_queries():
    ...
    async with KnowledgeBase() as kb:
        ...
        categories = ["engineering", "design", "product", "marketing", "sales", "hr", "finance", "legal"]
        ...
        print(f"✅ Setup complete: {await kb.count_nodes()} nodes, {await kb.count_edges()} edges")
        ...
        query_tests = [
            ...
            ("Single property", lambda: kb.query_nodes(properties={"department": "engineering"})),
            ("Multiple properties", lambda: kb.query_nodes(properties={"level": "L3", "active": True})),
            ...
            ("Connected nodes", lambda: kb.query_connected_nodes(
