...
class BenchmarkRunner:
    ...
    async def _benchmark_basic_operations(self):
        ...
        async with KnowledgeBase() as kb:
            ...
            print(
                f"✅ Node insertion: {node_insert_avg:.3f}ms avg, {node_insert_p95:.3f}ms p95"
            )
            ...
            print(
                f"✅ Edge insertion: {edge_insert_avg:.3f}ms avg, {edge_insert_p95:.3f}ms p95"
            )
        ...
        async with KnowledgeBase() as kb:
            ...
            for i in range(10000):
                await kb.insert_node(
                    Node(label="BulkNode", properties={"batch": "bulk_test", "id": i})
                )
            end = time.perf_counter()
            ...
            print(
                f"✅ Bulk insert (10k nodes): {bulk_insert_time:.2f}s ({bulk_insert_rate:.0f} ops/sec)"
            )
    ...
    async def _benchmark_query_performance(self):
        ...
        async with KnowledgeBase() as kb:
            ...
            queries = [
                ...
                (
                    "Relationship query",
                    lambda: kb.query_edges(relationship="reports_to"),
                ),
            ]
    ...
    async def _benchmark_rollback_performance(self):
        ...
        async with KnowledgeBase() as kb:
            ...
            for scenario_name, steps in rollback_scenarios:
                ...
                rollback_rate = (
                    steps / (rollback_time / 1000)
                    if rollback_time > 0
                    else float("inf")
                )

                print(
                    f"✅ {scenario_name} rollback: {rollback_time:.2f}ms ({rollback_rate:.0f} ops/sec)"
                )

                # Restore state for next test
                for i in range(steps):
                    await kb.insert_node(
                        Node(label="RollbackTest", properties={"id": f"restore_{i}"})
                    )
    ...
    async def _benchmark_storage_tiers(self):
        ...
        async with KnowledgeBase() as kb:
            ...
            prune_rate = (
                pruned / (prune_time / 1000) if prune_time > 0 else float("inf")
            )

            print(
                f"✅ Pruning: {prune_time:.2f}ms, moved {pruned} items ({prune_rate:.0f} items/sec)"
            )
            print(
                f"✅ After pruning - Runtime: {await kb.count_nodes(tier='runtime')}, Store: {await kb.count_nodes(tier='store')}"
            )
            ...
            print(
                f"✅ Cross-tier query: {cross_tier_query_time:.2f}ms for {len(all_nodes.nodes)} nodes"
            )
    ...
    async def _benchmark_concurrent_operations(self):
        ...
        async with KnowledgeBase() as kb:
            ...
            await asyncio.gather(
                *[
                    insert_batch(i, 200)
                    for i in range(10)  # 10 concurrent batches of 200
                ]
            )
            end = time.perf_counter()
            ...
            print(
                f"✅ Concurrent inserts (10 batches × 200): {concurrent_time:.2f}s ({concurrent_rate:.0f} ops/sec)"
            )
    ...
    async def _benchmark_memory_usage(self):
        ...
        async with KnowledgeBase() as kb:
            ...
            print(
                f"✅ Memory per node: {(peak_memory - initial_memory) / 10000 * 1024:.1f} KB"
            )
    ...
    async def _benchmark_scaling_characteristics(self):
        ...
        for size in data_sizes:
            async with KnowledgeBase() as kb:
                ...
                for i in range(size):
                    await kb.insert_node(
                        Node(label="ScaleTest", properties={"id": i, "size": size})
                    )
                ...
                scaling_results[size] = {
                    ...
                    "query_rate": len(result.nodes) / query_time
                    if query_time > 0
                    else float("inf"),
                }

                print(
                    f"✅ Size {size:5d}: Insert {insert_time:.3f}s ({size / insert_time:.0f} ops/sec), "
    ...
    def print_industry_comparison(self):
        ...
        print(
            f"  Momo KB node insert:     {our_results.get('node_insert_avg_ms', 0):.3f}ms"
        )
        print(
            f"  Neo4j node insert:       {industry_standards['neo4j_node_insert_ms']:.3f}ms"
        )
        print(
            f"  PostgreSQL insert:       {industry_standards['postgresql_insert_ms']:.3f}ms"
        )
        print(
            f"  SQLite insert:           {industry_standards['sqlite_insert_ms']:.3f}ms"
        )

        print(f"\n📊 Query Performance Comparison:")
        query_results = self.results.get("query_performance", {})
        print(
            f"  Momo KB query:           {query_results.get('label_query_avg_ms', 0):.3f}ms"
        )
        print(f"  Redis GET:               {industry_standards['redis_get_ms']:.3f}ms")
        print(
            f"  Elasticsearch query:     {industry_standards['elasticsearch_query_ms']:.3f}ms"
        )

        print(f"\n📊 Throughput Comparison:")
        bulk_results = self.results.get("bulk_operations", {})
        print(
            f"  Momo KB bulk insert:     {bulk_results.get('bulk_insert_rate_ops_per_sec', 0):.0f} ops/sec"
        )
