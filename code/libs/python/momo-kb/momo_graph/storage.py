...
class GraphGraphStorageTier(str, Enum):
...
class GraphThreeTierStorage:
    ...
    def __init__(self):
        ...
        self._nodes: Dict[GraphGraphStorageTier, Dict[str, GraphNode]] = {
            GraphGraphStorageTier.RUNTIME: {},
            GraphGraphStorageTier.STORE: {},
            GraphGraphStorageTier.COLD: {},
        }
        
        self._edges: Dict[GraphGraphStorageTier, Dict[str, GraphEdge]] = {
            GraphGraphStorageTier.RUNTIME: {},
            GraphGraphStorageTier.STORE: {},
            GraphGraphStorageTier.COLD: {},
        }
        ...
        self._indexes: Dict[GraphGraphStorageTier, GraphIndexManager] = {
            GraphGraphStorageTier.RUNTIME: GraphIndexManager(),
            GraphGraphStorageTier.STORE: GraphIndexManager(),
            GraphGraphStorageTier.COLD: GraphIndexManager(),
        }
        
    def count_nodes(self, tier: GraphGraphStorageTier) -> int:
        """Count nodes in a specific tier."""
        return len(self._nodes[tier])
        
    def count_edges(self, tier: GraphGraphStorageTier) -> int:
        """Count edges in a specific tier."""
        return len(self._edges[tier])
        
    def add_node(self, node: GraphNode, tier: GraphGraphStorageTier) -> None:
        """Add a node to a specific tier."""
        self._nodes[tier][node.id] = node
        self._indexes[tier].add_node(node)
        
    def add_edge(self, edge: GraphEdge, tier: GraphGraphStorageTier) -> None:
        """Add an edge to a specific tier."""
        self._edges[tier][edge.id] = edge
        self._indexes[tier].add_edge(edge)
        
    def get_node(self, node_id: str, tier: GraphGraphStorageTier) -> Optional[GraphNode]:
        """Get a node from a specific tier."""
        return self._nodes[tier].get(node_id)
        
    def get_edge(self, edge_id: str, tier: GraphGraphStorageTier) -> Optional[GraphEdge]:
    ...
    def move_node(self, node_id: str, from_tier: GraphStorageTier, to_tier: GraphStorageTier) -> bool:
    ...
    def move_edge(self, edge_id: str, from_tier: GraphStorageTier, to_tier: GraphStorageTier) -> bool:
    ...
    def query_nodes_indexed(
        ...
        node_ids = self._indexes[tier].query_nodes_indexed(label=label, properties=properties)
        return [self._nodes[tier][node_id] for node_id in node_ids if node_id in self._nodes[tier]]
    ...
    def query_edges_indexed(
        ...
        if relationship is None and source_id is None and target_id is None and not properties:
            return list(self._edges[tier].values())
        ...
        return [self._edges[tier][edge_id] for edge_id in edge_ids if edge_id in self._edges[tier]]
    ...
    def query_connected_nodes_indexed(
        ...
        return [self._nodes[tier][node_id] for node_id in connected_node_ids if node_id in self._nodes[tier]]