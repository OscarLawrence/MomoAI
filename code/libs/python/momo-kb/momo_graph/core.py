"""
Core Graph Backend implementation with immutable operations and rollback.

Implements the graph backend interface with diff-based versioning and 3-tier storage.
"""
...
from .models import GraphNode, GraphEdge, GraphGraphDiff, GraphGraphGraphDiffType, GraphGraphQueryResult
from .storage import GraphGraphStorageTier, GraphGraphThreeTierStorage
...
class GraphBackend:
    ...
    - GraphDiff-based rollback system
    - 3-tier storage with automatic pruning
    - Agent-optimized query pipeline
    """
    
    def __init__(self):
        self._storage = GraphThreeTierStorage()
        self._diff_history: List[GraphDiff] = []
    ...
    async def count_nodes(self, tier: Optional[str] = None) -> int:
        ...
        if tier is None:
            total = 0
            for storage_tier in [GraphStorageTier.RUNTIME, GraphStorageTier.STORE, GraphStorageTier.COLD]:
                total += self._storage.count_nodes(storage_tier)
            return total
        else:
            return self._storage.count_nodes(GraphStorageTier(tier))
    ...
    async def count_edges(self, tier: Optional[str] = None) -> int:
        ...
        if tier is None:
            ...
            return self._storage.count_edges(GraphStorageTier(tier))
            
    async def insert_node(self, node: Node) -> GraphDiff:
        """Insert a node and create diff record."""
        # Add to runtime tier (hottest)
        self._storage.add_node(node, GraphStorageTier.RUNTIME)
        
        # Create diff record
        diff = GraphDiff(operation=GraphGraphDiffType.INSERT_NODE, node=node)
        self._diff_history.append(diff)
        
        return diff
        
    async def insert_edge(self, edge: Edge) -> GraphDiff:
        """Insert an edge and create diff record."""
        # Add to runtime tier (hottest)
        self._storage.add_edge(edge, GraphStorageTier.RUNTIME)
        
        # Create diff record
        diff = GraphDiff(operation=GraphGraphDiffType.INSERT_EDGE, edge=edge)
        self._diff_history.append(diff)
        
        return diff
        
    async def delete_node(self, node_id: str) -> GraphDiff:
        ...
        for tier in [GraphStorageTier.RUNTIME, GraphStorageTier.STORE, GraphStorageTier.COLD]:
            self._storage.remove_node(node_id, tier)
            
        # Create diff record
        diff = GraphDiff(operation=GraphGraphDiffType.DELETE_NODE, node=node)
        self._diff_history.append(diff)
        
        return diff
        
    async def delete_edge(self, edge_id: str) -> GraphDiff:
        ...
        # Create diff record
        diff = GraphDiff(operation=GraphGraphDiffType.DELETE_EDGE, edge=edge)
        self._diff_history.append(diff)
        
        return diff
        
    async def get_diff_history(self) -> List[GraphDiff]:
    ...
    async def _apply_diff(self, diff: GraphDiff) -> None:
        """Apply a diff operation to the storage."""
        if diff.operation == GraphGraphDiffType.INSERT_NODE and diff.node:
            self._storage.add_node(diff.node, GraphStorageTier.RUNTIME)
        elif diff.operation == GraphGraphDiffType.DELETE_NODE and diff.node:
            for tier in [GraphStorageTier.RUNTIME, GraphStorageTier.STORE, GraphStorageTier.COLD]:
                self._storage.remove_node(diff.node.id, tier)
        elif diff.operation == GraphGraphDiffType.INSERT_EDGE and diff.edge:
            self._storage.add_edge(diff.edge, GraphStorageTier.RUNTIME)
        elif diff.operation == GraphGraphDiffType.DELETE_EDGE and diff.edge:
            for tier in [GraphStorageTier.RUNTIME, GraphStorageTier.STORE, GraphStorageTier.COLD]:
    ...
    async def query_nodes(
        ...
        for tier in [GraphStorageTier.RUNTIME, GraphStorageTier.STORE, GraphStorageTier.COLD]:
            # Fast indexed query
            nodes = self._storage.query_nodes_indexed(tier, label=label, properties=properties)
            ...
            if updated_nodes and tier != GraphStorageTier.RUNTIME:
    ...
    async def query_edges(
        ...
        for tier in [GraphStorageTier.RUNTIME, GraphStorageTier.STORE, GraphStorageTier.COLD]:
            ...
            if updated_edges and tier != GraphStorageTier.RUNTIME:
                storage_tier = tier.value
