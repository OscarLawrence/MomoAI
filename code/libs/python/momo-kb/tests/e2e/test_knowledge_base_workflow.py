...
class TestKnowledgeBaseWorkflow:
    ...
    async def test_multi_tier_storage_workflow(self, kb):
        ...
        for i in range(3):
            node = await kb.insert_node(
                Node(label="HotData", properties={"id": i})
            )
        ...
        for i in range(5):
            node = await kb.insert_node(
                Node(label="ColdData", properties={"id": i})
            )
    ...
    async def test_complex_query_workflow(self, kb):
        ...
        # People
        alice = await kb.insert_node(
            Node(label="Person", properties={"name": "Alice", "department": "Engineering"})
        )
        bob = await kb.insert_node(
            Node(label="Person", properties={"name": "Bob", "department": "Design"})
        )
        charlie = await kb.insert_node(
            Node(label="Person", properties={"name": "Charlie", "department": "Engineering"})
        )
        ...
        await kb.insert_edge(
            Edge(source_id=alice.node.id, target_id=project_x.node.id, relationship="works_on")
        )
        await kb.insert_edge(
            Edge(source_id=bob.node.id, target_id=project_x.node.id, relationship="works_on")
        )
        await kb.insert_edge(
            Edge(source_id=alice.node.id, target_id=charlie.node.id, relationship="mentors")
        )
