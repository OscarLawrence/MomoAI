...
class TestSemanticDataModels:
    ...
    def test_node_embedding_immutability(self):
        ...
        original_node = Node(label="Document", properties={"title": "Test Doc"})
...
class TestVectorIndex:
    ...
    def test_remove_vectors(self, vector_index):
        ...
        embeddings = [[0.1, 0.2] + [0.0] * 382, [0.3, 0.4] + [0.0] * 382]
...
class TestSemanticSearch:
    ...
    async def test_semantic_node_search(self, semantic_search):
        ...
        result = await semantic_search.search_nodes(query=query, top_k=5, threshold=0.7)
    ...
    async def test_find_similar_nodes(self, semantic_search):
        ...
        reference_node = Node(
            label="Document", properties={"title": "Deep Learning Fundamentals"}
        )

        result = await semantic_search.find_similar_nodes(
            reference_node=reference_node, top_k=3, exclude_self=True
        )
    ...
    async def test_contextual_search(self, semantic_search):
        ...
        result = await semantic_search.contextual_search(
            context=conversation_context, max_results=15
        )
...
class TestSemanticIntegration:
    ...
    async def test_performance_with_embeddings(self, kb_with_semantic):
        ...
        for i in range(100):
            node = Node(
                label="TestNode", properties={"index": i, "category": f"cat_{i % 10}"}
            )
            await kb_with_semantic.insert_node(node)
    ...
    async def test_backward_compatibility(self, kb_with_semantic):
        ...
        alice = await kb_with_semantic.insert_node(
            Node(label="Person", properties={"name": "Alice"})
        )
        bob = await kb_with_semantic.insert_node(
            Node(label="Person", properties={"name": "Bob"})
        )

        edge = await kb_with_semantic.insert_edge(
            Edge(source_id=alice.node.id, target_id=bob.node.id, relationship="knows")
        )
        ...
        friends = await kb_with_semantic.query_connected_nodes(
            start_node_id=alice.node.id, relationship="knows", direction="outgoing"
        )
