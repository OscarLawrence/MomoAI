...
class TestQueryCorrectness:
    ...
    async def populated_kb(self):
        ...
        alice = await kb.insert_node(Node(
            label="Person", 
            properties={"name": "Alice", "age": 30, "department": "Engineering", "active": True}
        ))
        bob = await kb.insert_node(Node(
            label="Person",
            properties={"name": "Bob", "age": 25, "department": "Design", "active": True}
        ))
        charlie = await kb.insert_node(Node(
            label="Person",
            properties={"name": "Charlie", "age": 35, "department": "Engineering", "active": False}
        ))
        
        # Companies
        techcorp = await kb.insert_node(Node(
            label="Company",
            properties={"name": "TechCorp", "size": "large", "industry": "technology"}
        ))
        designco = await kb.insert_node(Node(
            label="Company", 
            properties={"name": "DesignCo", "size": "small", "industry": "design"}
        ))
        
        # Projects
        project_x = await kb.insert_node(Node(
            label="Project",
            properties={"name": "ProjectX", "status": "active", "budget": 100000}
        ))
        project_y = await kb.insert_node(Node(
            label="Project",
        ...
        await kb.insert_edge(Edge(
            source_id=alice.node.id, target_id=techcorp.node.id, 
            relationship="works_for", properties={"since": 2020}
        ))
        await kb.insert_edge(Edge(
            source_id=bob.node.id, target_id=designco.node.id,
            relationship="works_for", properties={"since": 2021}
        ))
        await kb.insert_edge(Edge(
            source_id=charlie.node.id, target_id=techcorp.node.id,
            relationship="worked_for", properties={"from": 2018, "to": 2023}
        ))
        
        await kb.insert_edge(Edge(
            source_id=alice.node.id, target_id=project_x.node.id,
            relationship="assigned_to", properties={"role": "lead"}
        ))
        await kb.insert_edge(Edge(
            source_id=bob.node.id, target_id=project_x.node.id,
            relationship="assigned_to", properties={"role": "designer"}
        ))
        await kb.insert_edge(Edge(
            source_id=alice.node.id, target_id=project_y.node.id,
            relationship="completed", properties={"role": "developer"}
        ))
        
        await kb.insert_edge(Edge(
            source_id=alice.node.id, target_id=bob.node.id,
            relationship="mentors", properties={"area": "technical"}
        ))
    ...
    async def test_multiple_property_query_correctness(self, populated_kb):
        ...
        active_engineers = await kb.query_nodes(properties={
        ...
        inactive_engineers = await kb.query_nodes(properties={
            "department": "Engineering",
            "active": False
        })
        ...
        young_designers = await kb.query_nodes(properties={
            "department": "Design",
            "age": 25
        })
        ...
        old_designers = await kb.query_nodes(properties={
            "department": "Design",
            "age": 50
        })
    ...
    async def test_edge_query_correctness(self, populated_kb):
        ...
        for edge in work_edges.edges:
            source_node = await kb.query_nodes(properties={"id": edge.source_id})  # This won't work, need different approach
    ...
    async def test_connected_nodes_query_correctness(self, populated_kb):
        ...
        assert len(all_relationships.nodes) == 1  # Alice mentors Bob, no one mentors Alice
