...
class TestDataIntegrity:
    ...
    async def test_node_uniqueness_and_identity(self, kb):
        ...
        # Create a node
        original = await kb.insert_node(Node(
            label="TestNode",
    ...
    async def test_edge_referential_integrity(self, kb):
        ...
        # Create edge
        edge = await kb.insert_edge(Edge(
    ...
    async def test_rollback_data_integrity(self, kb):
        ...
        edge1 = await kb.insert_edge(Edge(
            source_id=node1.node.id,
            target_id=node2.node.id,
            relationship="connects"
        ))
        ...
        edge2 = await kb.insert_edge(Edge(
    ...
    async def test_large_property_values(self, kb):
        ...
        node = await kb.insert_node(Node(
            ...
            properties={
                ...
                "nested_data": {
                    "level1": {
                        "level2": {
                            "data": large_list[:100]  # Subset for JSON serialization
                        }
                    }
                }
            }
        ...
        assert found_node.properties["nested_data"]["level1"]["level2"]["data"] == large_list[:100]
    ...
    async def test_null_and_empty_values(self, kb):
        ...
        for i, props in enumerate(test_cases):
            node = await kb.insert_node(Node(
                label="EmptyValueNode",
                properties={**props, "test_id": i}
            ))
