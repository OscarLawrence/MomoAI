...
class TestIndexedVsNaive:
    ...
    async def large_test_kb(self):
        ...
        for i in range(
            0, len(people_nodes) - 1, 3
        ):  # Every 3rd person mentors the next
            await kb.insert_edge(
                Edge(
    ...
    async def test_node_queries_match_naive(self, large_test_kb):
        ...
        for i, test_case in enumerate(test_cases):
            ...
            assert indexed_ids == naive_ids, (
                f"Mismatch in test case {i + 1}: {test_case}"
            )
            assert len(indexed_result.nodes) == len(naive_result), (
                f"Count mismatch in test case {i + 1}"
            )
    ...
    async def test_edge_queries_match_naive(self, large_test_kb):
        ...
        for i, test_case in enumerate(test_cases):
            ...
            assert len(indexed_result.edges) == len(naive_result), (
                f"Count mismatch in test case {i + 1}"
            )
    ...
    async def test_connected_queries_match_naive(self, large_test_kb):
        ...
        test_node_ids = [
            people.nodes[i].id for i in [0, 5, 10, 15] if i < len(people.nodes)
        ]
        ...
        for node_id in test_node_ids:
            for i, test_case in enumerate(test_cases):
                print(
                    f"Testing connected query for node {node_id[:8]}..., case {i + 1}: {test_case}"
                )
                ...
                assert indexed_ids == naive_ids, (
                    f"Mismatch for node {node_id}, case {i + 1}: {test_case}"
                )
