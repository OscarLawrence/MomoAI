...
class EmbeddingManager:
    ...
    async def generate_embedding(
        self, text: str, model: str = "openai-ada-002"
    ) -> List[float]:
    ...
    async def embed_node_content(
        self, node: Node, model: str = "openai-ada-002"
    ) -> Node:
...
class SemanticSearch:
    ...
    async def search_nodes(
        ...
        # Filter by threshold
        filtered_results = [
            (node_id, score) for node_id, score in similar_results if score >= threshold
        ]
        ...
        for node_id, score in filtered_results:
            ...
            node = Node(
                id=node_id, label="MockNode", properties={"semantic_search": True}
            )
            nodes.append(node)
            scores.append(score)
    ...
    async def find_similar_nodes(
        self, reference_node: Node, top_k: int = 5, exclude_self: bool = True
    ) -> SemanticQueryResult:
        """Find nodes similar to a reference node."""
        if reference_node.embedding is None:
            # Generate embedding if not present
            embedded_node = await self.embedding_manager.embed_node_content(
                reference_node
            )
        ...
        # Search for similar embeddings
        similar_results = self.vector_index.similarity_search(
            query_embedding, k=top_k + 1
        )
    ...
    async def get_recommendations(
        self, interests: List[str], top_k: int = 10, diversity_threshold: float = 0.8
    ) -> SemanticQueryResult:
    ...
    async def contextual_search(
        self, context: List[str], max_results: int = 15
    ) -> SemanticQueryResult:
