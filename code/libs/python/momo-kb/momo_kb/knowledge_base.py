...
class KnowledgeBase(KnowledgeBaseCore):
    ...
    async def insert(self, *docs) -> List[str]:
        ...
        if not self._initialized:
            raise RuntimeError(
                "KnowledgeBase not initialized. Use async context manager."
            )
        ...
        for doc in docs:
            ...
            if isinstance(unified_item, Document):
                for backend_name, backend_id in backend_ids.items():
                    unified_item = unified_item.with_backend_id(
    ...
    async def search(
        self, query: str, filters: Optional[Dict] = None, top_k: int = 10, **kwargs
    ) -> SearchResult:
        """
    ...
    async def prune(
        self, mode: str = "auto", threshold: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        # Record operation
        self._record_operation("prune", mode=mode, threshold=threshold, results=results)
    ...
    async def screenshot(self) -> str:
        ...
        backend_counts = [
            stats.backend_stats.get(name, {}).get("nodes", 0) for name in backend_names
        ]
        ...
        if stats.document_types:
            ax2.pie(
                stats.document_types.values(),
                labels=stats.document_types.keys(),
                autopct="%1.1f%%",
            )
