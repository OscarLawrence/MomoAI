...
@click.option(
    "--init-config", is_flag=True, help="Initialize configuration in current directory"
)
@click.option("--config", type=click.Path(), help="Specify config file path")
@click.option("--verbose", "-v", is_flag=True, help="Verbose output")
@click.option(
    "--ai-output/--raw-output", default=True, help="Use AI-tailored output formatting"
)
@click.option(
    "--output-format",
    type=click.Choice(["structured", "json", "markdown"]),
    default="structured",
...
def script(ctx, script_name: str, args):
    ...
    executor = CommandExecutor(
        config, ctx.obj.get("verbose", False), ctx.obj.get("ai_output", True)
    )
...
def _suggest_similar_scripts(discovery: ScriptDiscovery, script_name: str):
    ...
    suggestions = [
        ...
        if script_name.lower() in s.lower() or s.lower() in script_name.lower()
    ]
