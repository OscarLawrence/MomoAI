...
class OutputFormatter:
    ...
    def format_output(self, command: str, stdout: str, stderr: str, returncode: int) -> CommandOutput:
        ...
        formatted_output = self._apply_command_formatting(command, (cleaned_lines, filtered_count), status)
        
        if formatted_output:
            return formatted_output
        
        # Default formatting
        return self._format_default_output(command, (cleaned_lines, filtered_count), status)
    ...
    def _clean_and_filter_lines(self, lines: List[str]) -> Tuple[List[str], int]:
        ...
        filtered_count = sum(max(0, count - self.duplicate_threshold) 
                           for count in seen_hashes.values())
        
        return cleaned, filtered_count
    
    def _apply_command_formatting(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> Optional[CommandOutput]:
        """Apply command-specific formatting."""
    ...
    def _format_default_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
    ...
    def _format_test_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
        ...
        important_lines = []
        for line in lines:
            if any(keyword in line.lower() for keyword in ['failed', 'error', 'assertion', 'traceback']):
    ...
    def _format_build_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
        ...
        important_lines = []
        for line in lines:
            if any(keyword in line.lower() for keyword in ['error', 'warning', 'built', 'compiled']):
                important_lines.append(line)
        
        summary = f"Build: {status} - {build_info.get('artifacts', 'unknown')} artifacts"
    ...
    def _format_pytest_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
        ...
        return CommandOutput(
            ...
            total_lines=len(lines),
            filtered_duplicates=filtered_count,
            expandable_body='\n'.join(failure_lines[7:]) if len(failure_lines) > 7 else None,
            metadata=test_results
        )
    
    def _format_nx_test_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
        """Format Nx test command output."""
        return self._format_test_output(command, lines_and_filtered, status, filtered_count)
    
    def _format_nx_build_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
        """Format Nx build command output."""
        return self._format_build_output(command, lines_and_filtered, status, filtered_count)
    
    def _format_lint_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
        ...
        summary = f"Lint: {len(issues)} issues found" if issues else "Lint: No issues found"
    ...
    def _format_create_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
    ...
    def _format_install_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
        ...
        packages = []
        for line in lines:
            if '+' in line and any(keyword in line for keyword in ['installed', 'added', 'updated']):
    ...
    def _format_npm_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
        """Format npm command output."""
        return self._format_install_output(command, lines_and_filtered, status, filtered_count)
    
    def _format_uv_output(self, command: str, lines_and_filtered: Tuple[List[str], int], status: str) -> CommandOutput:
