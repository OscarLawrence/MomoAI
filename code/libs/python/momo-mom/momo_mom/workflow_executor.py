...
class WorkflowCommandExecutor(CommandExecutor):
    ...
    def __init__(
        self, config: Dict[str, Any], workflow_context: Optional[Dict[str, Any]] = None
    ):
    ...
    def execute_workflow_command(
        self, command: str, step_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _format_command(self, command: str, context: Dict[str, Any]) -> str:
        ...
        try:
            return command.format(
                module=module, feature=feature, pattern=pattern, **context
            )
    ...
    def _execute_with_fallback(
        ...
        fallback_command = self._get_fallback_command(
            command, step_type, result["error"]
        )
        ...
        recovery_result = self._apply_recovery(
            command, step_type, module, result["error"]
        )
        if recovery_result:
    ...
    def _get_fallback_command(
        self, original_command: str, step_type: str, error: str
    ) -> Optional[str]:
        ...
        if "nx run" in original_command:
            if "install" in original_command:
                return original_command.replace(
                    "nx run", "cd code/libs/python"
                ).replace(":install", " && uv sync")
            elif "format" in original_command:
                return original_command.replace(
                    "nx run", "cd code/libs/python"
                ).replace(":format", " && uv run ruff format .")
            elif "lint" in original_command:
                return original_command.replace(
                    "nx run", "cd code/libs/python"
                ).replace(":lint", " && uv run ruff check .")
            elif "test" in original_command:
                return original_command.replace(
                    "nx run", "cd code/libs/python"
                ).replace(":test-fast", " && uv run pytest")
    ...
    def _apply_recovery(
        self, command: str, step_type: str, module: str, error: str
    ) -> Optional[Dict[str, Any]]:
        """Apply recovery strategies for common workflow issues"""

        # Nx cache issues
        if "nx" in error.lower() and (
            "cache" in error.lower() or "daemon" in error.lower()
        ):
        ...
        if "module not found" in error.lower() or "import error" in error.lower():
            ...
            if module:
                sync_result = self._safe_execute(
                    f"cd code/libs/python/{module} && uv sync --reinstall"
                )
    ...
    def validate_environment(self, module: str) -> Dict[str, Any]:
        ...
        validation_results["ready"] = all(
            [
                ...
                validation_results["nx_available"]
                or validation_results["uv_available"],
...
class WorkflowAgentIntegration:
    ...
    def execute_workflow_step(self, step_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        context = self.interactive_system.create_execution_context(
            current_task=step_info.get("description", ""), session_metadata=step_info
        )
