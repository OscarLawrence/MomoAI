...
class MomBenchmarkSuite:
    ...
    def _benchmark_agent_selection(self):
        ...
        for command, expected_agent in test_cases:
            ...
            try:
                ...
                print(
                    f"  {command:30} -> {actual_agent:15} {status} ({execution_time * 1000:.1f}ms)"
                )
        ...
        for command, agent_name, test_prompt, description in test_cases:
            ...
            try:
                ...
                if agent and agent.name == agent_name:
                    ...
                    print(
                        f"  {agent_name:15} {test_prompt:20} -> '{response[:30]}...' {status}"
                    )
    ...
    def _generate_report(self) -> Dict[str, Any]:
        ...
        agent_matches = sum(
            1 for r in self.results if r.agent_match and r.expected_agent != "N/A"
        )
        ...
        agent_match_rate = (
            agent_matches / total_matchable * 100 if total_matchable > 0 else 0
        )
        ...
        print(
            f"  Agent Matches:      {agent_matches}/{total_matchable} ({agent_match_rate:.1f}%)"
        )
        ...
        for test_type, results in test_types.items():
            ...
            print(
                f"  {test_type:20} -> {len(results):3} tests, {success_rate:5.1f}% success, {avg_time * 1000:6.1f}ms avg"
            )
        ...
        print(
            f"  Executing Agent:    {'✅' if self.system.registry.executing_agent else '❌'}"
        )
        print(f"  Specialized Agents: {len(self.system.registry.specialized_agents)}")
        print(
            f"  General Agent:      {'✅' if self.system.registry.general_agent else '❌'}"
        )
        ...
        report = {
            ...
            "system_info": {
                ...
                "agent_names": [
                    agent.name for agent in self.system.registry.get_all_agents()
                ],
                "executing_agent_available": self.system.registry.executing_agent
                is not None,
                "specialized_agents_count": len(
                    self.system.registry.specialized_agents
                ),
                "general_agent_available": self.system.registry.general_agent
                is not None,
            },
